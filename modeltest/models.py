...
class Link(models.Model):    
    ...
    create_dt = models.DateTimeField(auto_now_add=True, blank=True)    
    
    def __str__(self):
        return self.title
    
class LinkDomain(models.Model):
    link = models.ForeignKey(Link, on_delete=modelsRESTRI.CASCADE)    
    url = models.URLField(unique=True)
    response = models.JSONField(null=True)
    create_dt = models.DateTimeField(auto_now_add=True, blank=True)       
...
class LinkeMeta(models.Model):
    TYPE = (
        ('name','name'),
        ('property','property'),
    )    
    article = models.ForeignKey(Link, on_delete=models.CASCADE)
    type = models.CharField(max_length = 50, choices=TYPE)
    key = models.CharField(max_length = 50)
    value = models.CharField(max_length = 255)
    full_text = models.CharField(max_length=1024)
    create_dt = models.DateTimeField(auto_now_add=True, blank=True)
