...
def index(request):
    # latest_organization_list = Organization.objects.order_by("-pub_date")[:5]
    # context = {"latest_organization_list": latest_organization_list}
    return render(request, "sitesearch/index.html")

def search(request):
    search_links = Link.objects.order_by("title")
    context = {"search_links": search_links}
    return render(request, "sitesearch/search.html", context)
